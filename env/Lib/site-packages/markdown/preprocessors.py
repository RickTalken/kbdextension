    ...
    def _nested_markdown_in_html(self, items):
        ...
        for i, item in enumerate(items):
            if self.left_tag_re.match(item):
                left_tag, left_index, attrs = self._get_left_tag("".join(items[i:]))
        ...
        while text:
            ...
            if not in_tag:
                if block.startswith("<") and len(block.strip()) > 1:
                    ...
                    if not (
                        self.md.is_block_level(left_tag)
                        or block[1] in ["!", "?", "@", "%"]
                    ):
                ...
                if self._equal_tags(left_tag, right_tag):
                    ...
                    if self.markdown_in_raw and "markdown" in attrs.keys():
                        ...
                        self.md.htmlStash.tag_data[-1 - nests]["right_index"] += (
                            nests - 2
                        )
                    else:
